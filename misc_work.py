...
from rymscraper import (
    rymscraper,
    RymUrl,
)  # W: 'rymscraper.RymUrl' imported but unused # W: Unused RymUrl imported from rymscraper
...
def get_recommendations(album_list, full_dataframe):
    ...
    subset_df = full_dataframe[
        full_dataframe["genres"].apply(lambda x: any(item in x for item in combined_genres))
    ]
